...
test()
{
  ...
  // ── assemble only the cell Jacobian block ────────────────────────────────
  ...
  problem.assemble_jacobian_cell_block(t, problem.solution);

  // ── central FD check ─────────────────────────────────────────────────────
  // Step size scales with the magnitude of each DOF to handle both
  // large (area ~1e-4 m^2) and small (velocity ~0) variables.
  ...
  for (unsigned int j = 0; j < n_total; ++j)
    {
      ...
      Fp.reinit(n_total);
      problem.assemble_cell_residuals(t, yp, Fp);
      Fm.reinit(n_total);
      problem.assemble_cell_residuals(t, ym, Fm);
