...
test()
{
  ...
  problem.solution = 1.0; // Velocity = 1 everywhere, A = 1 everywhere

  problem.assemble_mass_matrix();

  double L = problem.mass_matrix.matrix_scalar_product(problem.solution,
                                                       problem.solution);

  deallog << "1 + 1 = " << L << std::endl;
}
