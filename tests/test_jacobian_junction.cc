...
test()
{
  ...
  // ── assemble only the trace-junction Jacobian block ─────────────────────
  ...
  problem.assemble_jacobian_trace_junction_block(problem.solution);
  ...
  for (unsigned int j = 0; j < n_total; ++j)
    {
     ...
     // deallog << j << " y=" << problem.solution[j] << " h=" << h << std::endl;
      ...
      Fp.reinit(n_total);
      problem.assemble_trace_junction_equations(yp, Fp);
      Fm.reinit(n_total);
      problem.assemble_trace_junction_equations(ym, Fm);
