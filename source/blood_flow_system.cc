...
template <int dim, int spacedim>
void
BloodFlowSystem<dim, spacedim>::output_csv(const double          t,
                                           const Vector<double> &y)
{
  // For each monitoring point we find the cell whose center is closest
  // to the target location, then read (A, U, P) from that cell's DOFs.

  const FEValuesExtractors::Scalar area_extractor(0);
  const FEValuesExtractors::Scalar vel_extractor(1);

  // Target locations in spacedim=3 coordinates
  // AortaMid  : midpoint of vessel 0  (x=0.043, y=0, z=0)
  // Junction  : bifurcation point     (x=0.086, y=0, z=0)
  // IliacMid  : midpoint of vessel 1  (x=0.128, y=0.043, z=0)
  //             (adjust coordinates to match your actual mesh)
  const Point<spacedim> p_aorta_mid(0.043, 0.0, 0.0);
  const Point<spacedim> p_junction(0.086, 0.0, 0.0);
  const Point<spacedim> p_iliac_mid(0.128, 0.043, 0.0);

  // Helper: find cell closest to a target point and extract (A, U, P)
  auto extract_at = [&](const Point<spacedim> &target,
                        double                &P_kPa,
                        double                &Q_ml_s,
                        double                &dr_mm) {
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
      {
        const double d = cell->center().distance(target);
        if (d < best_dist)
          {
            best_dist = d;
            best_cell = cell;
    ...
    // Average A and U over all DOFs of their component
    double       A_sum = 0.0, U_sum = 0.0;
    unsigned int nA = 0, nU = 0;
    for (unsigned int i = 0; i < fe->n_dofs_per_cell(); ++i)
      {
        const unsigned int comp = fe->system_to_component_index(i).first;
        if (comp == 0)
    ...
    const double A = A_sum / nA;
    const double U = U_sum / nU;

    // Pressure in kPa
    P_kPa = compute_pressure_value(A, vid) * 1e-3;

    // Flow rate Q = A * U in ml/s (1 m³/s = 1e6 ml/s)
    Q_ml_s = A * U * 1e6;

    // Change in radius from diastole in mm
    // r = sqrt(A/pi),  r_d = sqrt(a_d/pi)
    const double r   = std::sqrt(A / numbers::PI);
    const double r_d = std::sqrt(vpp.a_d / numbers::PI);
    dr_mm            = (r - r_d) * 1e3;
  };

  // Write one row to each CSV
  auto write_row = [&](std::ofstream &f, const Point<spacedim> &target) {
    double P, Q, dr;
    extract_at(target, P, Q, dr);
    f << std::scientific << std::setprecision(6) << t << "," << P << "," << Q
      << "," << dr << "\n";
    f.flush();
  };

  write_row(csv_aorta_mid_, p_aorta_mid);
  write_row(csv_junction_, p_junction);
  write_row(csv_iliac_mid_, p_iliac_mid);
}
  // ============================================================================
  // HLL flux (residual)
  // ============================================================================
  ...
  // ============================================================================
  template <int dim, int spacedim>
  void BloodFlowSystem<dim, spacedim>::update_terminal_pressures(
    const double dt, const Vector<double> &y)
  {
    // ---- 1. Accumulate flow at each terminal boundary ----------------------
    std::map<unsigned int, double> Q_accumulated;
    for (const auto &[bid, Pc] : terminal_Pc_storage)
      Q_accumulated[bid] = 0.0;

    // Extract cell sub-vector (FEFaceValues needs size == dof_handler.n_dofs())
    Vector<double> y_cell(n_cell_dofs);
    for (types::global_dof_index i = 0; i < n_cell_dofs; ++i)
      y_cell[i] = y[i];
      ...
      for (unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
        {
          ...
          fef.reinit(cell, f);
          fef[area_extractor].get_function_values(y_cell, A_vals);
          fef[velocity_extractor].get_function_values(y_cell, U_vals);
          const auto &JxW = fef.get_JxW_values();
    ...
    for (const auto &[bid, Pc_old] : terminal_Pc_storage)
      {
        ...
        Pc_new_map[bid]    = (Pc_old + (dt / rcr.C) * Q_tot +
                              (dt / (rcr.C * rcr.R2)) * rcr.P_out) /
                             denom;
      }

    // Write back separately
    for (auto &[bid, Pc] : terminal_Pc_storage)
      Pc = Pc_new_map[bid];
  }
        ...
        for (double tt : {0.0, 0.14, 0.28, 0.46, 0.68, 0.827})
          {
            ...
            std::cout << "  Q(" << std::setw(5) << tt
                      << ") = " << std::scientific << Q << " m^3/s"
                      << "  (" << Q * 1e6 << " ml/s)\n";
          }
        std::cout << "-----------------------------\n";
