  ...
  for (const auto &[target_vid, target_pt] : probe_targets_)
    {
      ...
      const double P_Pa =
        compute_pressure_value(A_val, vid, compute_a_d_local(best_cell));
  ...
  for (unsigned int cycle = 0; cycle < n_refinement_cycles; ++cycle)
    {
      ...
      for (const auto &[vid, vp] : vessel_map)
        {
          ...
          const double c_in =
            std::sqrt(beta / (2.0 * par["rho"] * a_in)) * std::pow(a_in, 0.25);
      ...
      //   //trace equations are zero at initial state, by doing
      //   initialize_trace_unknow() we don't need them
