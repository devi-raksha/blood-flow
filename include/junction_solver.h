...
public:
  ...
  solve(const JunctionState &X0,
        const double         Wp_plus,
        const double         Wd1_minus,
        const double         Wd2_minus,
        const Physics       &phys)
  {
    ...
    for (unsigned int it = 0; it < max_iter; ++it)
      {
        ...
        // --------------------------------------------------
        phys.compute_junction_residual(X, Wp_plus, Wd1_minus, Wd2_minus, R);
        ...
        while (alpha > 1e-6)
          {
            ...
            if (X_trial.Ap > Amin && X_trial.Ad1 > Amin && X_trial.Ad2 > Amin)
              break;
